...
module.exports = {
  getLikedRecipeByUser: async (req, res) => {
    try {
      ...
      const totalData = await likedRecipeModel.getCountLikedRecipe(id);
      ...
      const result = await likedRecipeModel.getLikedRecipeByUser(
        id,
        sort,
        sortType,
        limit,
  ...
  createLikedRecipe: async (req, res) => {
    try {
      ...
      return wrapper.response(
        res,
        200,
        `Success create liked recipe id ${data.id}`,
        result
      );
    } catch (error) {
      return wrapper.response(res, 400, `Bad Request : ${error.message}`, null);
